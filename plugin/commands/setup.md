---
description: Run wclaw setup (Claude auth + Telegram bot configuration)
---

Run the wclaw setup wizard. Since Claude Code's terminal doesn't support interactive stdin, orchestrate the setup conversationally by collecting values from the user and passing them as CLI flags.
...
If this fails, tell the user to build from source with `./build.sh` in the wclaw repo.
...
Ask the user:
> To set up wclaw, I need your Telegram bot token. If you don't have one yet, create a bot via [@BotFather](https://t.me/BotFather) on Telegram and copy the token it gives you.
...
```bash
CC_BIN="${HOME}/.wclaw/bin/wclaw"; [ -x "$CC_BIN" ] || CC_BIN="${CLAUDE_PLUGIN_ROOT}/bin/wclaw"; "$CC_BIN" setup --token "<TOKEN>" --yes
```
...
After setup completes successfully, suggest running `/wclaw:start` to launch the daemon.
