---
description: Diagnose and fix common wclaw issues
disable-model-invocation: true
---

Run the wclaw doctor to check for common issues. Execute each check below and report results:

## 1. Binary exists and is executable
```bash
CC_BIN="${HOME}/.wclaw/bin/wclaw"
if [ -x "$CC_BIN" ]; then
    echo "OK: binary found at $CC_BIN"
elif [ -x "${CLAUDE_PLUGIN_ROOT}/bin/wclaw" ]; then
    CC_BIN="${CLAUDE_PLUGIN_ROOT}/bin/wclaw"
    echo "OK: binary found at $CC_BIN (stable path missing — run /wclaw:update to fix)"
else
    echo "FAIL: binary not found — run /wclaw:setup to install"
fi
```
...
## 4. Config exists
```bash
DATA_DIR="${WCLAW_DATA_DIR:-$HOME/.wclaw}"
test -f "${DATA_DIR}/config.json" && echo "OK: config found at ${DATA_DIR}/config.json" || echo "FAIL: no config — run /wclaw:setup"
```
...
## 7. Daemon status
```bash
CC_BIN="${HOME}/.wclaw/bin/wclaw"; [ -x "$CC_BIN" ] || CC_BIN="${CLAUDE_PLUGIN_ROOT}/bin/wclaw"; "$CC_BIN" statusline 2>/dev/null || echo "Daemon not running"
```
...
if [ -f "${DATA_DIR}/daemon.lock" ]; then
    if pgrep -f "wclaw$" >/dev/null 2>&1; then
