#!/usr/bin/env bash
# Download the wclaw binary for the current platform from GitHub releases.
#
# Usage:
...
# --- Download & extract -------------------------------------------------------
ASSET_NAME="wclaw-${TARGET}.tar.gz"
DOWNLOAD_URL="https://github.com/${REPO}/releases/download/${VERSION}/${ASSET_NAME}"

echo "Downloading wclaw ${VERSION} for ${TARGET}..."
...
mv "${TMP_DIR}/wclaw" "$BIN"
chmod +x "$BIN"

echo "Installed wclaw ${VERSION} to ${BIN}"

# --- Also install to stable path (~/.wclaw/bin/) -------------------------
...
STABLE_DIR="${HOME}/.wclaw/bin"
mkdir -p "$STABLE_DIR"
cp "$BIN" "${STABLE_DIR}/wclaw"
chmod +x "${STABLE_DIR}/wclaw"
echo "Installed wclaw ${VERSION} to ${STABLE_DIR}/wclaw"
