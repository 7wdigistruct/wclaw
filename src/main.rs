...
async fn main() -> Result<()> {
    tracing_subscriber::fmt()
        .with_env_filter(
            EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| EnvFilter::new("wclaw=info,warn")),
        )
        .init();
    ...
    match std::env::args().nth(1).as_deref() {
        ...
        Some("hook-handler") => {
            ...
            let file_timeout =
                tokio::task::spawn_blocking(move || config::read_approval_timeout(&data_dir))
                    .await
                    .unwrap_or(None);
            let env_timeout = std::env::var("WCLAW_APPROVAL_TIMEOUT")
                .ok()
                .and_then(|v| match v.parse::<u64>() {
                    Ok(n) => Some(n),
                    Err(_) => {
                        eprintln!(
                            "Warning: WCLAW_APPROVAL_TIMEOUT is invalid, falling back to config/default"
                        );
                        None
                    }
                });
        ...
        Some("stop") => {
            if is_daemon_running() {
                signal_stop();
                eprintln!("Sent stop signal to wclaw daemon");
            } else {
                eprintln!("No wclaw daemon is running");
            }
            return Ok(());
        }
        Some("help" | "--help" | "-h") => {
            eprintln!("Usage: wclaw [command]");
            eprintln!();
            eprintln!("Commands:");
        ...
        Some(cmd) => {
            ...
            eprintln!("Run `wclaw help` for usage.");
            std::process::exit(1);
        }
        None => {}
    }

    tracing::info!("Starting wclaw daemon v{}", env!("CARGO_PKG_VERSION"));
        ...
        if let Err(e) = f.try_lock() {
            match e {
                std::fs::TryLockError::WouldBlock => {
                    eprintln!("Error: another wclaw instance is already running");
                    eprintln!(
    ...
    if !config_path.exists() {
        ...
        if !config_path.exists() {
            anyhow::bail!("Setup did not create config. Run `wclaw setup` to try again.");
        }
    }
    ...
    // WCLAW_SYSTEM_PROMPT overrides the default. This is intentional:
    ...
    let (permission_server, permission_handle) = if config.telegram_approval {
        ...
        // scoped to the wclaw subprocess only.
...
/// Parse `wclaw setup` flags: `--token <T>`, `--yes`.
...
/// Update wclaw to the latest release using install-binary.sh.
async fn run_update() -> Result<()> {
    ...
    let plugin_status = std::process::Command::new("claude")
        .args(["plugin", "update", "wclaw@wclaw"])
        .status();
...
fn find_install_script() -> Option<std::path::PathBuf> {
    ...
    let cache_base = std::path::PathBuf::from(&home)
        .join(".claude/plugins/cache/wclaw/wclaw");
    ...
    // Fallback: relative to exe (source builds: plugin/bin/wclaw)
...
async fn check_for_update(data_dir: &std::path::Path) -> Result<()> {
    ...
    let output = tokio::process::Command::new("curl")
        .args(["-fsSL", "--max-time", "5",
               "https://api.github.com/repos/7wdigistruct/wclaw/releases/latest"])
        .output()
        .await?;
...
async fn start_daemon() -> Result<()> {
    ...
    let config_path = config::config_path(&data_dir);
    if !config_path.exists() {
        anyhow::bail!("No config found. Run `wclaw setup` first.");
    }
    ...
    if lock_path.exists() {
        if let Ok(f) = std::fs::File::open(&lock_path) {
            if f.try_lock().is_err() {
                eprintln!("wclaw is already running");
                return Ok(());
            }
        }
    }
    ...
    let log = std::fs::File::create("/tmp/wclaw.log")?;
    ...
    let child = cmd.spawn()?;
    eprintln!("wclaw started (pid: {})", child.id());
    Ok(())
}
