...
///
/// Uses the stable binary path at `~/.wclaw/bin/wclaw` which is
...
pub fn install_statusline(data_dir: &Path) {
    ...
    let stable_bin = data_dir.join("bin").join("wclaw");
    ...
    match load_status(status_path) {
        ...
        Some(s) => {
            ...
            if let Some(latest) = &update_hint {
                print!(
                    "\n{d}│{r}  {cyan}⬆ {latest} available{r} {d}— /wclaw:update{r}",
                    cyan = cyan(c),
                );
            }
        }
