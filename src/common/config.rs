...
fn default_system_prompt() -> String {
    "You are wclaw, a personal AI assistant daemon. You are being controlled remotely via Telegram. \
...
impl DaemonConfig {
    ...
    fn apply_env_overrides(&mut self) {
        if let Some(v) = std::env::var("WCLAW_TELEGRAM_TOKEN").ok().filter(|s| !s.is_empty()) {
            self.telegram_token = v;
        }
        if let Some(v) = std::env::var("WCLAW_ADMIN_CHAT_ID")
            .ok()
            .and_then(|s| match s.parse() {
                Ok(id) => Some(id),
                Err(_) => {
                    tracing::warn!(value = %s, "WCLAW_ADMIN_CHAT_ID is not a valid integer, ignoring");
                    None
                }
            })
        {
        ...
        if let Some(v) = std::env::var("WCLAW_UPDATE_CHECK_INTERVAL")
            .ok()
            .and_then(|s| match s.parse() {
                Ok(n) => Some(n),
                Err(_) => {
                    tracing::warn!(value = %s, "WCLAW_UPDATE_CHECK_INTERVAL is not a valid integer, ignoring");
                    None
                }
            })
        {
...
/// Resolve the data directory. Precedence: `WCLAW_DATA_DIR` env > `~/.wclaw`
pub fn data_dir() -> PathBuf {
    if let Ok(v) = std::env::var("WCLAW_DATA_DIR") {
        PathBuf::from(v)
    } else if let Some(home) = dirs::home_dir() {
        home.join(".wclaw")
    } else {
        eprintln!("Warning: cannot determine home directory, falling back to ./.wclaw");
        PathBuf::from(".wclaw")
    }
}
...
mod tests {
    ...
    async fn validate_rejects_missing_working_directory() {
        ...
        let json = serde_json::json!({
            "telegram_token": "123:ABC",
            "admin_chat_id": 42,
            "working_directory": "/definitely/missing/wclaw-working-dir"
        })
        .to_string();
        tokio::fs::write(&path, json).await.unwrap();
        let err = DaemonConfig::load(&path).await.unwrap_err();
