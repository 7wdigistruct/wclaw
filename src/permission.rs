...
/// `wclaw hook-handler` — called by Claude Code's `PreToolUse` hook.
///
...
pub async fn handle_hook(socket_path: &Path, timeout_secs: u64) -> Result<()> {
    ...
    .await
    {
        ...
        Ok(Ok(_)) => {
            eprintln!("wclaw: server closed connection without a decision");
            print!(
                "{}",
        ...
        Ok(Err(e)) => {
            eprintln!("wclaw: read error from daemon ({e})");
            print!(
                "{}",
        ...
        Err(_) => {
            eprintln!("wclaw: timed out waiting for permission decision");
...
/// wclaw passes that file via `claude --settings`, so global
/// `~/.claude/settings.json` is irrelevant for wclaw subprocesses.
                ...
                entry
                    ...
                    .is_some_and(|hooks| {
                        hooks.iter().any(|h| {
                            h.get("command")
                                .and_then(|c| c.as_str())
                                .is_some_and(is_wclaw_hook_command)
                        })
                    })
            })
        })
...
/// subprocess via `--settings`, keeping it scoped to wclaw only.
pub fn install_hook(working_dir: &Path) -> Result<()> {
    ...
    let exe_path = std::env::current_exe().context("Cannot determine path to wclaw binary")?;
...
/// Check whether a command string refers to the wclaw hook-handler binary.
///
/// Matches commands written by `install_hook`: `'<path>/wclaw' hook-handler`
/// or `<path>/wclaw hook-handler`. Strips quotes from the binary path before
/// checking the basename.
fn is_wclaw_hook_command(cmd: &str) -> bool {
...
mod tests {
    ...
    // -- is_wclaw_hook_command --

    #[test]
    fn hook_command_single_quoted_path() {
        assert!(is_wclaw_hook_command(
            "'/usr/local/bin/wclaw' hook-handler"
        ));
    }

    #[test]
    fn hook_command_unquoted_bare_name() {
        assert!(is_wclaw_hook_command("wclaw hook-handler"));
    }

    #[test]
    fn hook_command_path_with_spaces_single_quoted() {
        assert!(is_wclaw_hook_command(
            "'/home/user/my apps/wclaw' hook-handler"
        ));
    }
    ...
    fn hook_command_slash_prefix() {
        assert!(is_wclaw_hook_command("/usr/local/bin/wclaw hook-handler"));
    }

    #[test]
    fn hook_command_rejects_not_wclaw() {
        assert!(!is_wclaw_hook_command("not-wclaw hook-handler"));
    }

    #[test]
    fn hook_command_rejects_missing_hook_handler() {
        assert!(!is_wclaw_hook_command("'/usr/bin/wclaw'"));
        assert!(!is_wclaw_hook_command("wclaw"));
    }

    #[test]
    fn hook_command_rejects_wrong_subcommand() {
        assert!(!is_wclaw_hook_command("wclaw statusline"));
        assert!(!is_wclaw_hook_command("wclaw setup"));
    }

    #[test]
    fn hook_command_rejects_no_wclaw() {
        assert!(!is_wclaw_hook_command("something-else hook-handler"));
        assert!(!is_wclaw_hook_command(""));
    }

    #[test]
    fn hook_command_rejects_prefixed_shell_command() {
        assert!(!is_wclaw_hook_command("echo wclaw hook-handler"));
    }
