...
# Now copy real source and do an incremental rebuild (only wclaw crate recompiles).
COPY src/ src/
RUN touch src/main.rs && cargo build --release && strip target/release/wclaw
...
RUN useradd -m -s /bin/bash -u 1000 wclaw

COPY --from=builder /build/target/release/wclaw /usr/local/bin/wclaw
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
  CMD wclaw statusline 2>/dev/null | head -1 | grep -qv "offline" || exit 1
